CREATE OR REPLACE VIEW CLIENTE AS 
	SELECT
		"CardCode" AS "IDCLIENTEERP",
		0 AS "IDCIDADEERP",
		'ativo'  AS "IDSITUACAOERP",
		0 AS "IDREGIAOERP",
		0 AS "IDATIVIDADEERP",
		0 AS "IDRAMOERP",
		"CardName"  AS "NOME",
		"CardName"  AS "RSOCIAL",
		"Address"  AS "ENDERECO",
		"Number"  AS "NUMERO",
		"Block"  AS "BAIRRO",
		"ZipCode"  AS "CEP",
		'' AS "IERG",
		'' AS "CNPJCPF",
		"Phone1"  AS "TELEFONE",
		"Fax"  AS "FAX",
		"Cellular"  AS "CELULAR",
		"E_Mail"  AS "EMAIL",
		"E_Mail"  AS "EMAILNFE",
		"U_Rov_Data_Nascimento"  AS "DATANASCIMENTO",
		"Free_Text"  AS "OBSCADASTRAL",
		'' AS "OBSFINANCEIRA",
		'0' AS "IDTABPRECOERP",
--		'' AS "IDTABPRECOERP",
--		'' AS "IDTABPRECOERP",
		0 AS "IDFORMAPAGTOERP",
		"PaymBlock"  AS "IDPRAZOPAGTOERP",
		0 AS "COEFTABPRECO",
		0 AS "DESCONTOMAXIMO",
		0 AS "DATAHORA",
		1 AS "TIPOCONSUMIDOR",
		0 AS "CONTRIBUINTEICMS",
		0 AS "FARMPOPULAR",
		0 AS "CODOPERACAOCOM",
		0 AS "COMPLEMENTO",
		0 AS "IDENDERECOENTREGAPADRAO",
		0 AS "IDENDERECOCOBRANCAPADRAO",
		'N' AS "RECEBIMENTOPARCIAL",
		0 AS "IDMATRIZ",
		0 AS "IGNORAREGRAFLEX",
		0 AS "LATITUDE",
		0 AS "LONGITUDE",
		0 AS "PDOT",
		0 AS "VALORMINIMOFOB",
		0 AS "VALORMINIMOCIF",
		0 AS "DATAULTIMAVENDA",
		0 AS "DATAHORAATUALIZACAO",
		0 AS "CONTRIBUINTE",
		0 AS "ESTRANGEIRO",
		0 AS "TIPOCLIENTE",
		0 AS "PRAZOENTREGA",
		0 AS "FREQUENCIACOMPRA",
		0 AS "ESTOQUESEGURANCA",
		0 AS "IDTIPOPEDIDOERP",
		0 AS "IDTRANSPORTADORAERP",
		0 AS "IDCLI_GRUPOECONOMICOERP"
	FROM
		OCRD;
		
	
SELECT
	CRD2.*
FROM
	OCRD
	LEFT JOIN CRD2 ON (OCRD."CardCode" = CRD2."CardCode")
WHERE ocrd."CardCode" = 'CLI0000231';
	


SELECT * FROM CRD2 WHERE "LineNum" > 0


SELECT * FROM CLIENTE WHERE "IDCLIENTEERP" = 'FOR0000425'





