...
SELECT DISTINCT
	...
	CAST(CASE
		...
		WHEN OQUT."DocStatus" = 'C'
		AND OQUT.CANCELED = 'Y' THEN 'Cancelado'
...
LEFT JOIN USUARIO ON
	"SlpCode" = "IDUSUARIOERP"
INNER JOIN QUT1 ON
	OQUT."DocEntry" = QUT1."DocEntry"
LEFT JOIN ODRF ON
	OQUT."U_id_pedido_forca" = ODRF."U_id_pedido_forca"
LEFT JOIN ORDR pedido ON
	OQUT."U_id_pedido_forca" = pedido."U_id_pedido_forca"
LEFT JOIN OINV fatura ON
	OQUT."U_id_pedido_forca" = fatura."U_id_pedido_forca"
WHERE
